...
function propertyHeroTwoCol(ctx, includeShowingAgent = true) {
  ...
  const statusLine = esc(ctx.status);
...
export function applicationNotificationEmail({
  ...
  return wrap(`
    ...
    <table role="presentation" style="width:100%;border-spacing:0;font-size:14px;color:#444444;">
      ...
      <tr><td style="padding:6px 0;color:#666666;vertical-align:top;"><strong>Property ID</strong></td><td style="padding:6px 0;">${esc(property_name)}</td></tr>
...
`;
  ...
  function row(label, value) {
    return `
    <tr>
      <td style="${tdStyle}; width:40%; font-weight:600;">${esc(label)}</td>
      <td style="${tdStyle};">${esc(value || "")}</td>
    </tr>
  `;
  }
  ...
  function section(title, rows) {
    return `
    <h3 style="margin:20px 0 8px; font-size:14px; color:#555; text-decoration: underline;">${title}</h3>
  ...
  const summary = `
  <p style="margin:0 0 16px;font-size:18px;font-weight:bold;color:#333;">
    New Applying Details
  </p>

  ${section("Property & Move-in Details", `
    ${row("Property Name", fields.property_name)}
    ${row("Move in Date", fields.move_in_date)}
    ${row("Applicant Type", fields.applicant_type)}
  `)}
  ...
  ${section("Personal Information", `
    ...
    ${row("Hear About Us", fields.hear_about)}
  `)}

  ${section("Previous Residence", `
    ${row("Address 1", fields.address1)}
    ${row("Address 2", fields.address2)}
    ${row("City", fields.city)}
    ${row("State", fields.state)}
    ${row("Zipcode", fields.zipcode)}
    ${row("Monthly Rent", fields.monthly_rent)}
    ${row("Landlord Name", fields.landlord_name)}
    ${row("Landlord Phone", fields.landlord_phone)}
  `)}

  ${section("Housemates & Pets", `
    ${row("Other Adults", fields.has_adults)}
    ${row("Adult Count", fields.adult_count)}
    ${row("Adult Names", fields.adult_names)}
    ${row("Adult Emails", fields.adult_emails)}
    ${row("Adult Phones", fields.adult_phones)}
    ${row("Has Pets", fields.has_pets)}
    ${row("Pet Count", fields.pet_count)}
  `)}

  ${section("Personal Info & Vehicle", `
    ${row("Date of Birth", fields.dob)}
    ${row("License Number", fields.license_number)}
    ${row("Personal Email", fields.personal_email)}
    ${row("Personal Phone", fields.personal_phone)}
  `)}

  ${section("Employment & Income", `
    ${row("Employer Name", fields.employer_name)}
    ${row("Monthly Salary", fields.monthly_salary)}
    ${row("Position", fields.position)}
  `)}

  ${section("Background Information", `
    ${row("Eviction History", fields.eviction_history)}
    ${row("Criminal History", fields.criminal_history)}
  `)}

  ${section("Billing Information", `
    ${row("Billing Name", fields.billing_first_name + " " + fields.billing_last_name)}
    ${row("Billing Address", fields.billing_address1)}
    ${row("Billing City", fields.billing_city)}
    ${row("Billing State", fields.billing_state)}
    ${row("Billing Zip", fields.billing_zip)}
  `)}
  ...
  ${section("Payment Information", `
    ...
    ${row("Amount Paid", "$50")}
    ${row("Stripe Charge ID", fields.stripe_id)}
  `)}

  ${section("Authorization", `
    ${row("Authorized Name", fields.authorized_name)}
    ${row("Agreed Terms", fields.agree_terms)}
  `)}

  <p style="margin-top:20px;font-size:13px;">
    Thank you,<br/>
    The Purple Housing Team
  </p>
`;
  ...
  //   [
  //     adminRow("Property ID", f.property_name),
...
export function applicationThankYouEmail({
  ...
  return wrap(`
    ...
    <table style="width:100%;border-spacing:0;font-size:14px;margin-top:16px;">
      ${adminRow("Property ID", property_name)}
