...
import { blogSlugFromTitle } from "./blogSlug";
...
export async function searchPropertiesAdvanced({
  ...
  let conditions = `WHERE status != 'expire'`;

  // 🔍 Search
  if (query) {
    conditions += ` AND (
      prop_title LIKE '%${query}%' 
      OR property_map_address LIKE '%${query}%'
      OR city LIKE '%${query}%'
    )`;
  }
  ...
  if (homeType && homeType !== "Any Type") {
    conditions += ` AND category LIKE '%${homeType}%'`;
  }

  if (beds && beds !== "Any") {
    if (beds === "5+") {
      conditions += ` AND CAST(prop_beds AS UNSIGNED) >= 5`;
    } else if (!isNaN(Number(beds))) {
      conditions += ` AND CAST(prop_beds AS UNSIGNED) = ${Number(beds)}`;
    }
  }

  if (baths && baths !== "Any" && !isNaN(Number(baths))) {
    conditions += ` AND CAST(prop_baths AS UNSIGNED) = ${Number(baths)}`;
  }
  ...
  if (priceRange) {
    if (priceRange.includes("-")) {
      ...
      conditions += ` AND CAST(prop_price AS UNSIGNED) BETWEEN ${min} AND ${max}`;
    } else if (priceRange.includes("+")) {
      const min = priceRange.replace("+", "");
      conditions += ` AND CAST(prop_price AS UNSIGNED) >= ${min}`;
    }
  }
  ...
  return {
    rows: rows.map(serializePrismaRow),
...
/** Resolve by title slug (URL segment). Falls back to numeric id for old links. */
export async function getBlogBySlug(slug) {
  const raw = String(slug || "").trim();
  if (!raw) return null;

  if (/^\d+$/.test(raw)) {
    const byId = await getBlogById(raw);
    if (byId) return byId;
  }

  const rows = await prisma.blogs.findMany({ orderBy: { id: "desc" } });
  for (const row of rows) {
    const blog = enrichBlog(serializePrismaRow(row));
    if (blogSlugFromTitle(blog.title) === raw) return blog;
  }
  return null;
}
...
/* ─── Bookings ─── */
/** Match booking `property_name` (prop_title) to a listing for email image, link, and status. */
export async function getPropertyForBookingEmail(propTitle) {
  const title = String(propTitle || "").trim();
  ...
  return row ? enrichProperty(serializePrismaRow(row)) : null;
}
...
export async function getSortedProperties({
  ...
  // ✅ DEFAULT = LOW → HIGH
  let orderByRaw = `CAST(prop_price AS UNSIGNED) ASC`;

  if (sort === "price_high_low") {
    orderByRaw = `CAST(prop_price AS UNSIGNED) DESC`;
  }

  if (sort === "price_low_high") {
    orderByRaw = `CAST(prop_price AS UNSIGNED) ASC`;
  }
